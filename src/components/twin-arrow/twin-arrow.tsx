  ...
  function TwinArrow(
    ...
    useEffect(() => {
      ...
      async function init() {
        ...
        // Determine which element triggers the hover
        const trigger = hoverParent
          ? (container.parentElement ?? container)
          : container;

        const d = distance;
        const stagger = 0; // delay between exit and entry

        // Initial state: original visible at center, clone hidden at bottom-left
        gsap.set(original, { x: 0, y: 0, opacity: 1 });
        gsap.set(clone, { x: -d, y: d, opacity: 0 });

        const onEnter = () => {
          // 1. Original exits toward top-right (↗)
          gsap.to(original, {
            x: d,
            y: -d,
            opacity: 0,
          ...
          // 2. Clone enters from bottom-left after stagger delay
          gsap.to(clone, {
            x: 0,
            y: 0,
            opacity: 1,
        ...
        const onLeave = () => {
          // 1. Clone exits back to bottom-left
          gsap.to(clone, {
            x: -d,
            y: d,
            opacity: 0,
          ...
          // 2. Original returns to center after stagger delay
          gsap.to(original, {
            x: 0,
            y: 0,
            opacity: 1,
